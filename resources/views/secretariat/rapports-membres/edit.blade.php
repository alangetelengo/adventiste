...
@section('page-title', __('secretariat.rapports_membres.title_edit'))

@section('page-title-info')
    {{ $typesPeriode[$rapport->type_periode] ?? $rapport->type_periode }} — {{ $rapport->annee }} @if($rapport->mois > 0) {{ __('secretariat.rapports_membres.edit_subtitle_month', ['month' => $rapport->mois]) }} @endif
@endsection
...
@section('content')
    <div class="max-w-2xl adventiste-card-pro-static p-6">
        <form method="post" action="{{ route('secretariat.rapports-membres.update', $rapport) }}" class="space-y-6" data-offline-queue>
            ...
            <div>
                <label for="notes_locales" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{{ __('secretariat.rapports_membres.notes_label') }}</label>
            ...
            <label class="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                ...
                {{ __('secretariat.rapports_membres.recalculate_stats') }}
            </label>
