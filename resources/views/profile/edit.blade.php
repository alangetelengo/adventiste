...
@section('page-title-info')
    ...
    <span class="text-slate-600 dark:text-slate-400">{{ __('profile.page_subtitle') }}</span>
@endsection
...
<div class="max-w-4xl mx-auto space-y-6">
    <dl class="adventiste-card-pro-static p-6 sm:p-7 grid gap-5 sm:grid-cols-2 text-sm">
        <div class="sm:col-span-2">
            <h2 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4 pb-3 border-b border-slate-200/80 dark:border-slate-600/80">{{ __('profile.personal_info') }}</h2>
        </div>
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('profile.full_name') }}</dt>
            <dd class="mt-1 font-medium text-slate-900 dark:text-white">{{ $user->name }}</dd>
        </div>
        <div>
            <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('profile.email') }}</dt>
            <dd class="mt-1 text-slate-800 dark:text-slate-200 break-all">{{ $user->email }}</dd>
        </div>
        @if($user->eglise_locale_id)
        <div class="sm:col-span-2">
            <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('profile.local_church') }}</dt>
            <dd class="mt-1 text-slate-800 dark:text-slate-200">{{ $user->egliseLocale?->nom ?? '—' }}</dd>
        </div>
        @endif
        @if($user->mission_id)
        <div class="sm:col-span-2">
            <dt class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('profile.mission') }}</dt>
    ...
    <div class="grid gap-6 md:grid-cols-2">
        <div class="adventiste-card-pro-static p-6 sm:p-7">
            <h2 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4">{{ __('profile.role_section') }}</h2>
            @if($user->role)
            ...
            @else
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('profile.no_roles') }}</p>
            @endif
        </div>

        <div class="adventiste-card-pro-static p-6 sm:p-7 flex flex-col">
            <h2 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4">{{ __('profile.activity') }}</h2>
            <div class="rounded-xl bg-slate-50/90 dark:bg-slate-900/50 border border-slate-200/70 dark:border-slate-600/60 px-4 py-5 text-center grow flex flex-col justify-center">
                ...
                <p class="mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('profile.member_since') }}</p>
            </div>
        </div>
    </div>

    <div class="adventiste-card-pro-static p-6 sm:p-7">
        <h2 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4">{{ __('profile.security') }}</h2>
        <p class="text-sm text-slate-600 dark:text-slate-400 mb-5 max-w-2xl">
            {{ __('profile.security_blurb') }}
        </p>
