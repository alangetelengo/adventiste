...
@section('page-title', 'Éditer entrée — ' . $groupe->nom)
...
<div class="adventiste-card-pro-static max-w-3xl p-6 sm:p-8">
    <p class="text-sm text-slate-600 dark:text-slate-400 mb-6 leading-relaxed border-b border-slate-200/80 dark:border-slate-600/60 pb-6">
        Modifier les montants de l'entrée financière.
    </p>
    <form method="post" action="{{ route('parametres.groupes-mission.entrees.update', [$groupe, $entree]) }}" class="space-y-8">
        @csrf
        @method('PUT')
        @include('parametres.groupes-mission.entrees._form')
