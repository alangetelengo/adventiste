...
@section('page-title', 'Nouvelle entrée — ' . $groupe->nom)
...
<div class="adventiste-card-pro-static max-w-3xl p-6 sm:p-8">
    <p class="text-sm text-slate-600 dark:text-slate-400 mb-6 leading-relaxed border-b border-slate-200/80 dark:border-slate-600/60 pb-6">
        Saisir les montants de l'entrée financière pour le mois et l'année sélectionnés.
    </p>
    <form method="post" action="{{ route('parametres.groupes-mission.entrees.store', $groupe) }}" class="space-y-8">
        @csrf
        @include('parametres.groupes-mission.entrees._form', ['entree' => null])
        <div class="flex flex-wrap gap-3 pt-2 border-t border-slate-200/80 dark:border-slate-600/60">
            ...
            <a href="{{ route('parametres.groupes-mission.entrees.index', $groupe) }}" class="adventiste-btn-secondary">Annuler</a>
        </div>
    </form>
</div>
@endsection
