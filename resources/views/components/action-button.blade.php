...
$titleDisplay = $title ?? match ($variant) {
    'edit' => __('modules.common.edit'),
    'view' => __('modules.common.view'),
    'delete' => __('modules.common.delete'),
    default => '',
};
$confirmMsgResolved = $confirmMessage ?? __('modules.common.confirm_delete_default');
$confirmTextResolved = $confirmText ?? __('modules.common.delete');
@endphp

@if ($variant === 'edit')
<a href="{{ $href }}" class="{{ $baseClasses }} {{ $editClasses }}" title="{{ $titleDisplay }}">
...
@elseif ($variant === 'view')
<a href="{{ $href }}" class="{{ $baseClasses }} {{ $viewClasses }}" title="{{ $titleDisplay }}">
    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
...
<form method="{{ $method }}" action="{{ $action }}" class="inline m-0">
    ...
    @method($method)
    @endif
    <button type="button" class="{{ $baseClasses }} {{ $deleteClasses }}" title="{{ $titleDisplay }}" onclick="flashAlert(@json($confirmMsgResolved), this.closest('form'), { icon: @json($confirmIcon), danger: true, confirmText: @json($confirmTextResolved) })" {{ $disabled ? 'disabled' : '' }}>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
