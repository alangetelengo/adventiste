...
<div class="grid gap-4 sm:grid-cols-2">
    @if (auth()->user()->eglise_locale_id !== null)
        <div class="sm:col-span-2">
            ...
            <p class="text-sm leading-snug text-slate-600 dark:text-slate-400">
                {{ __('modules.baptemes.church_prefix') }} <strong class="text-slate-800 dark:text-slate-200">{{ auth()->user()->egliseLocale?->nom }}</strong>
                <span class="font-mono text-xs text-slate-500">({{ auth()->user()->egliseLocale?->code_unique }})</span>
            </p>
        </div>
    @else
        <div class="sm:col-span-2">
            <label for="eglise_locale_id" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_local_church') }}</label>
    ...
    <div>
        <label for="nom" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_nom') }}</label>
    ...
    <div>
        <label for="prenom" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_prenom') }}</label>
    ...
    <div>
        <label for="type_bapteme" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_type') }}</label>
    ...
    <div>
        <label for="date_bapteme" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_date_bapteme') }}</label>
    ...
    <div>
        <label for="date_admission_eglise" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_date_admission') }}</label>
        ...
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('modules.baptemes.form_date_admission_hint') }}</p>
    ...
    <div>
        <label for="lieu_bapteme" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_lieu') }}</label>
    ...
    <div>
        <label for="officiant" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_officiant') }}</label>
    ...
    <div class="sm:col-span-2">
        <label for="notes" class="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1.5">{{ __('modules.baptemes.form_notes') }}</label>
...
<script>
    (function () {
        const loc = document.documentElement.lang || 'fr-FR';
        const toUpperIds = ['nom', 'officiant', 'lieu_bapteme'];
        const toTitleIds = ['prenom', 'notes'];

        function toUpper(value) {
            return (value || '').toLocaleUpperCase(loc);
        }

        function toTitle(value) {
            return (value || '')
                .toLocaleLowerCase(loc)
                .replace(/\b\p{L}/gu, (m) => m.toLocaleUpperCase(loc));
        }
